-- 0099_pool_shot_guards_follow_the_rack.sql
--
-- Multiplayer Pool stopped relaying: one player kept shooting and the other
-- never got a turn.
--
-- THE BUG. 0089 re-racked the table from nine object balls to fifteen and
-- derived the ball COUNT from pool_initial_metadata so it could not drift
-- again. It missed two other places that still spelled the old rack out by
-- hand, both inside submit_pool_shot:
--
--   1. The settled-ball id allowlist still read
--        not in ('cue','ball-1' ... 'ball-9')
--      so ball-10 through ball-15 — six balls the server itself racks — were
--      rejected as out of bounds. Every shot failed with 'settled ball
--      bounds invalid'. Not the first shot of a frame: EVERY shot.
--
--   2. `v_remaining := greatest(0, 9 - v_total_potted_count)` treated the
--      table as cleared at nine potted. The clear bonus is worth
--      500 + shotsRemaining * 50, and the client only pays it at fifteen, so
--      from the ninth pot onward the two disagreed and the shot was rejected
--      with 'score delta mismatch'. The same literal also ended the frame
--      six balls early through v_game_over.
--
-- Because a rejected shot never writes, currentSeat never advanced. The
-- shooter's own table keeps simulating locally, so from their side they go on
-- taking shot after shot, while the other player waits on a turn that can
-- never arrive and sees nothing move.
--
-- THE FIX. Both guards now read the rack instead of restating it. The valid
-- ids and the object-ball count come from pool_initial_metadata, the same
-- function that deals the table, so a future re-rack updates them for free.
-- Nothing else in the function changes.
--
-- Verified on a throwaway cluster: with the rack as it ships, the old
-- allowlist rejects ball-10..ball-15 and the old threshold pays a clear bonus
-- at nine pots; after this migration neither does.
