-- 0049_redeem_code_profile_owner.sql
--
-- Make redemption ownership explicit. `public.pets.owner_id` and
-- `public.redemption_code_redemptions.user_id` both reference
-- `public.profiles(id)`, so the redemption RPC should resolve/ensure the
-- caller's profile row before inserting rewards instead of relying on a
-- silent auth.uid() assumption.
