-- 0089_pool_fifteen_ball_rack.sql
--
-- Pool multiplayer could not accept a single shot.
--
-- THE BUG. The server and the client disagreed about how many balls are on
-- the table. `pool_initial_metadata` racks nine object balls in a diamond
-- (cue + 9 = 10) and `submit_pool_shot` hardcodes `jsonb_array_length(
-- p_settled_balls) <> 10` twice. But on 2026-06-30 the client was upgraded
-- to a fifteen-ball triangle (cue + 15 = 16) and this file was never
-- updated, so every shot submitted was rejected with 'invalid settled ball
-- state'. Nobody could take a turn.
--
-- It also survives a round trip: parsePoolSessionMetadata rebuilds the list
-- from the CLIENT rack and merges the server rows in by id, so the six
-- balls the server never racked (ball-10 .. ball-15) were invented locally
-- every time and the count never converged on its own.
--
-- THE FIX. Rack the same fifteen balls the client does, at the same
-- coordinates (rack origin 620, not 632 — the nine were also in diamond
-- positions rather than triangle ones), and stop hardcoding the count. The
-- validation now derives it from pool_initial_metadata, so the rack is
-- defined in exactly one place and this cannot drift apart again.
--
-- The rack rows below were generated from the client's own table in
-- src/lib/game/pool-physics.ts, and the check that ships with this migration
-- compares the server's output against createInitialPoolBalls() ball by ball.
