-- 0091_memory_match_bigger_board.sql
--
-- Two things: the watching player could never see the second card, and the
-- table grows from eight pairs to twelve.
--
-- THE BUG. When the second card of a turn is flipped, submit_game_move
-- resolves the pair and sets `revealed` back to '[]' in BOTH branches before
-- writing the metadata. The persisted state therefore never contains two
-- revealed cards. The player who flipped sees their own second card locally,
-- but the other player only ever renders from the server: they watch one
-- card turn over and then vanish, and on a miss they never see the second
-- card at all.
--
-- That removes the entire point of the game. Memory match is watching what
-- the other person turns over.
--
-- The fix is to record the pair rather than to keep `revealed` populated,
-- which would need a second round trip to clear and could wedge the board if
-- a player dropped mid-turn. `lastPair` is purely presentational: the client
-- shows those two cards for a beat on a miss and then turns them back down,
-- and a client that misses the update simply sees the next state.
--
-- THE BOARD. Twelve pairs, 24 cards, six columns by four rows. Three guards
-- had to move together, and the card-index one is the sharp edge: it
-- rejected anything above 15, so on a 24-card board the last eight cards
-- would have been unflippable with 'invalid card index'. Both bounds now
-- derive from memory_match_board_size() so the deck has one definition —
-- the same single-source rule 0089 applied to the Pool rack after the server
-- and client drifted to different table sizes.
--
-- The client deck in src/lib/game/memory-match-deck.ts carries the matching
-- twelve pairs, and MEMORY_MATCH_BOARD_SIZE is derived from that list.
...
comment on function public.memory_match_board_size() is
  'Cards on a Memory Match table. The deck builder and every bounds check in submit_game_move derive from this, so the board has exactly one definition. Must equal MEMORY_MATCH_BOARD_SIZE in src/lib/game/memory-match-deck.ts.';
...
declare
  v_deck text[] := array[
    'heart', 'petal', 'lantern', 'tree', 'casper', 'moon',
    'note', 'garden', 'basket', 'teatable', 'armchair', 'petbed',
    'heart', 'petal', 'lantern', 'tree', 'casper', 'moon',
    'note', 'garden', 'basket', 'teatable', 'armchair', 'petbed'
  ];
  v_size integer := public.memory_match_board_size();
  v_i integer;
  v_j integer;
  v_tmp text;
...
begin
  ...
  -- Fisher-Yates over the whole deck, bounded by the shared board size so a
  -- deck change cannot leave the tail of the array unshuffled.
...
begin
  ...
  if v_game_key = 'memory-match' then
    ...
    if v_board_arr is null or jsonb_typeof(v_board_arr) <> 'array' or jsonb_array_length(v_board_arr) < public.memory_match_board_size() then
    ...
    if v_card_index < 0 or v_card_index >= public.memory_match_board_size() then
