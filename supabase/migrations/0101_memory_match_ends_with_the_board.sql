-- 0101_memory_match_ends_with_the_board.sql
--
-- Memory Match ended with cards still face down.
--
-- THE BUG. 0036 dealt sixteen cards — eight pairs — and ended the game on
-- `v_match_count >= 8`. 0091 dealt twenty-four cards for a bigger table and
-- moved the index BOUNDS onto memory_match_board_size(), but the ending was a
-- separate literal and stayed at eight. Every game therefore stopped four
-- pairs early, with eight cards never turned over, and paid out a final score
-- computed from that short game.
--
-- THE FIX. The ending is read from the length of the board actually in play,
-- so it follows any table this or a future client deals — including a session
-- dealt at the old size, which still finishes correctly at eight.
--
-- Nothing else in submit_game_move changes.
...
begin
  ...
  if v_game_key = 'memory-match' then
    ...
    -- The board decides when the board is finished. This used to read
    -- `>= 8`, from the sixteen-card table of 0036; 0091 dealt twenty-four and
    -- left the literal behind, so every game ended with four pairs still
    -- face down. v_board_arr was re-read above, after any re-init, so it is
    -- this session's own table.
    v_game_over := v_match_count >= (jsonb_array_length(v_board_arr) / 2);
