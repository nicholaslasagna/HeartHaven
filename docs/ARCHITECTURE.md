...
Server Components own route composition, metadata, and data loading. Client Components are used only where browser APIs are needed: Framer Motion for the landing scene and Phaser for the room canvas.
...
The room renderer is isolated in `RoomCanvas` and dynamically imports Phaser 3 in the browser. Current mock placements mirror the `placed_items` table shape, so Phase 3 can replace static props with Supabase reads and save drag/drop edits without changing the screen contract.
